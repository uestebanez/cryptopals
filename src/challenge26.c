                     ...
                     size_t* cipher_len)
{
  ...
  wr+=pad;
  return aes128_ctr_crypt(plain,wr,g_key,g_iv,cipher,cipher_size,cipher_len);
}
...
bool is_admin(const uint8_t* cipher,size_t cipher_len)
{
  ...
  r = aes128_ctr_decrypt(cipher,cipher_len,g_key,g_iv,plain,sizeof(plain),
                         &plain_len);
  if( 0 != r ) {
